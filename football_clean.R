...
# CLEAN UP THIS SECTION + ADD DOCUMENTATION
...
fourth_sub <- fourth_sub %>% mutate(fourth_down_converted = factor(fourth_down_converted, levels = c('0','1'), labels = c('No','Yes'))
                                    ...
                                    ,passer = if_else(play_type == 'pass',if_else(is.na(passer) == TRUE, 'No Passer', passer),'Rush Attempt')
                                    ,rusher = if_else(is.na(rusher) == TRUE, 'No Rusher', rusher)
